...
function Skills() {
  return (
    <section id='skills' 
    className='py-16 px-[12vw] md:px-[7vw] lg:px-[20vw] font-sans bg-skills-gradient '
    > 
    ...
    <div className='flex flex-wrap gap-1 lg:gap-5'>
      {SkillsInfo.map((category) => (
        <div 
        key={category.title}
        className='bg-gray-900 backdrop-blur-md px-6 sm:px-10 py-8 sm:py-4 mb-10 w-full sm:w-[48%] shadow-lg rounded-2xl border border-white hover:shadow-xl hover:scale-105 transition-all duration-300'
        ...
        >
          ...
          >
            {category.skills.map((skill) => (
              <div
              key={skill.name}
              className='inline-flex items-center gap-2 bg-transparent border-2 border-gray-700 rounded-3xl py-2 px-2 min-w-[120px] max-w-[200px] justify-center '
              >
                <img src={skill.logo} alt={skill.name} className='h-4 w-4 sm:h-6 sm:w-6 ' />
